...
def total_revenue(data: dict, financial_index):
    ...
    if financial_index < len(data['financials']) and financial_index >=0:
      ...
      return float(financial['pnl']['lineItems']['net_revenue'])
...
def total_borrowing(data: dict, financial_index):
    ...
    if financial_index < len(data['financials']) and financial_index >=0:
      financial = data['financials'][financial_index]    
      total_borrowings = financial['bs']['liabilities']['long_term_borrowings'] + financial['bs']['liabilities']['short_term_borrowings']
      total_revenues = total_revenue(data, financial_index)
      ratio = float(total_borrowings/total_revenues)
      return ratio
...
def iscr(data: dict, financial_index):
    ...
    if financial_index < len(data['financials']) and financial_index >=0:
      financial = data['financials'][financial_index]    
      pbitd = (financial['pnl']['lineItems']['profit_before_interest_and_tax']+financial['pnl']['lineItems']['depreciation'])
      interest_expenses = financial['pnl']['lineItems']['interest']
      ISCR = float((pbitd+1)/(interest_expenses+1))
      return ISCR
...
def iscr_flag(data: dict, financial_index):
    ...
    """
    if financial_index < len(data['financials']) and financial_index >=0:
      ISCR = iscr(data, financial_index)
      flags = FLAGS()
      if ISCR>=2:
        return flags.GREEN
      else:
